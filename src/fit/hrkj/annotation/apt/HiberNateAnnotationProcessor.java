...
@SupportedAnnotationTypes({ "fit.hrkj.annotation.apt.Persistent", "fit.hrkj.annotation.apt.Id",
		"fit.hrkj.annotation.apt.Property" }) // 指定可处理的注解,需要加上注解的包名
public class HiberNateAnnotationProcessor extends AbstractProcessor {
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		if (!roundEnv.processingOver()) {
			processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE, "process方法执行了");//打印报告
		}
		// TODO Auto-generated method stub
		// 定义一个文件输出流，用于生成额外的文件
		PrintStream ps = null;
		try {
			...
			for (Element e : roundEnv.getElementsAnnotatedWith(Persistent.class)) {
				...
				ps.println("    <class name=\"" + e+"\""+" table=\"" + persistent.table() + "\">");
